...
fn test_docker_server_creation() {
    ...
    assert_eq!(
        ...
        Some("Docker MCP server: nginx".to_string())
    );
...
fn test_docker_server_command_generation() {
    ...
    config.insert(
        ...
        "POSTGRES_PASSWORD=secret123,POSTGRES_DB=testdb".to_string(),
    );
...
fn test_docker_server_with_volumes() {
    ...
    assert!(volume_args
        .iter()
        .any(|arg| arg.contains("/data:/var/lib/mysql")));
    assert!(volume_args
        .iter()
        .any(|arg| arg.contains("/config:/etc/mysql/conf.d")));
}
...
fn test_docker_server_metadata_fields() {
    ...
    assert!(field_names.iter().any(|name| name.contains("ports")
