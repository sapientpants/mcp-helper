...
fn test_invalid_config_format() {
    ...
    assert!(stderr.contains("Invalid config format") || 
...
fn test_batch_file_nonexistent() {
    ...
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("Failed to read batch file") || 
...
"#;
    ...
    assert!(stdout.contains("Found") || stdout.contains("Installing") || !stderr.contains("conflicting"));
}
